...
- BPM : 読み込み時に自動推定 (`src/core/bpm.ts`)。表示はテンポフェーダ込みの実効値。外したときは ×2 / ÷2 で直す
  - 手順はスペクトルフラックス → 区間ごとの自己相関を合算 → 支配的な周期の倍・半分だけを候補に → 128に近い倍率を採用
  - 倍率は波形からは決まらないため、実テンポが85〜87.5の曲は倍の値で出る
- EQ / FILTER : 64が素通し。絞り切り-26dB、上げ切り+6dB。FILTERは左でローパス、右でハイパス
- TEMPO : 中央で等速、端で±8%
- タンテ : 触れている間はジョグが速度を握る。逆に回せば逆再生。止まっているデッキでも擦れば鳴る
- 擦るボタン : BPMに合わせて1拍で1往復のジョグを自動で送る。信号は手で回したときと同じなのでPC連携でも効く
- PAD : ホットキュー4つ。未登録なら登録、登録済みなら頭出し、0.7秒の長押しで削除
- 再生 : `public/deck-processor.js` のAudioWorkletで読み出す。速度を負にできるので逆回転する
- 制約 : 音量とクロスフェーダーは `mapping.js` に番号が無く両デッキ等倍。ARは未対応
